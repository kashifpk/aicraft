...
## Known caveats (verified against Harbor 0.5)

- **Rootless Podman + claude-code session-file ownership.** Verified 2026-04-25: nop, codex+OpenRouter (deepseek-v4-flash and kimi-k2.6), and claude-code's *agent run itself* all complete cleanly on rootless podman — the original DNS-resolution failure no longer reproduces. The remaining issue is post-run trajectory ingestion for claude-code: its session JSONL files are written at mode `0600`, and rootless podman maps container root → a host subuid (e.g. `100999`), so the host user gets `Permission denied` reading them. The agent's reply is fine, only `_convert_claude_code_events_to_trajectory` fails. Workarounds: use Docker for claude-code, or chase `--userns=keep-id` upstream. Codex/nop/aider are unaffected — they write `0644`.
- **Harbor's codex agent strips the vendor prefix from `--model`.** `harbor/agents/installed/codex.py:644` does `self.model_name.split("/")[-1]`, so `moonshotai/kimi-k2.6` → `kimi-k2.6` and `deepseek/deepseek-v4-flash` → `deepseek-v4-flash` before being passed to the codex CLI. OpenRouter happens to accept the short form when unambiguous, so our tests passed — but the slug we send is not the slug Harbor delivers. If we ever route to a gateway that *requires* the vendor namespace, this breaks silently. Track upstream; consider a targeted monkey-patch if it bites.
...
- **LiteLLM imports eagerly during Harbor startup**, adding ~25s even for `nop` runs. Filed as upstream issue: https://github.com/harbor-framework/harbor/issues/1514 (still open as of 2026-04-27).
- **ATIF schema varies per agent**: codex emits ATIF-v1.5 (rich, with `tool_calls`/`observation`/`model_name` on steps), claude-code emits ATIF-v1.2 (minimal — just `step_id, timestamp, source, message`). Our `_extract_final_text` only uses fields present in v1.2, so it works for both. Anyone adding richer parsing (tool-call counts, edit-classification) needs to dispatch on `schema_version` or treat v1.5 fields as optional.

### Recently retired (Harbor 0.5)

- **`OPENAI_BASE_URL` not propagated to codex** — fixed upstream by [harbor#1482](https://github.com/harbor-framework/harbor/pull/1482) (merged 2026-04-23). Harbor 0.5's codex agent now writes `openai_base_url` to `config.toml` directly. Our `_harbor_patches.py` was deleted in the 0.1.1 bump.
