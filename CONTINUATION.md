...
- **Rootless Podman + claude-code session-file ownership.** Verified 2026-04-25: nop, codex+OpenRouter (deepseek-v4-flash and kimi-k2.6), and claude-code's *agent run itself* all complete cleanly on rootless podman against Harbor 0.4 — the original DNS-resolution failure no longer reproduces. The remaining issue is post-run trajectory ingestion for claude-code: its session JSONL files are written at mode `0600`, and rootless podman maps container root → a host subuid (e.g. `100999`), so the host user gets `Permission denied` reading them. The agent's reply is fine, only `_convert_claude_code_events_to_trajectory` fails. Workarounds: use Docker for claude-code, or chase `--userns=keep-id` upstream / via a monkey-patch like `_harbor_patches.py`. Codex/nop/aider are unaffected — they write `0644`.
- **Harbor's codex agent strips the vendor prefix from `--model`.** `harbor/agents/installed/codex.py:640` does `self.model_name.split("/")[-1]`, so `moonshotai/kimi-k2.6` → `kimi-k2.6` and `deepseek/deepseek-v4-flash` → `deepseek-v4-flash` before being passed to the codex CLI. OpenRouter happens to accept the short form when unambiguous, so our tests passed — but the slug we send is not the slug Harbor delivers. If we ever route to a gateway that *requires* the vendor namespace, this breaks silently. Track upstream; consider a targeted monkey-patch if it bites.
